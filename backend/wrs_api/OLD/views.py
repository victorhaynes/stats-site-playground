...
def basline_lp_and_ranked_history(request):
    ...
    baseline_lp_history.last_ranked_game = all_ranked_matches[0]
...
async def all_players(request):
    # all_puuids_and_last_games = RankedLpHistory.objects.values_list("puuid","last_ranked_game","region")
    urls = []
    vic_puuid="f649YxVWblcWTKMjnLYAZTKlbH6b3iNEZXf9-HXZhxxJRBSQ-Zws7v6jErBh0tzyVS9VNo50FHK3rA"
    abe_puuid="aYWKw--LdXozLfm-HJ8OFiHW9vKVUZLzwnsLvJFufYsmr4Xdjnac-3-u_2lRJPQZ6Z2Eop0EbJdOwQ"
    puuids = [vic_puuid, abe_puuid]
    # for record in all_puuids_and_last_games:
    for record in puuids:
        urls.append(f"https://{'americas'}.api.riotgames.com/lol/match/v5/matches/by-puuid/{record}/ids?start=0&count=1&queue=420"
)
    ...
    print(results[0].json())
    return JsonResponse(results[1].json()[0], safe=False)

# sync_to_asyc(asyncio.run(all_players()))
...
#     # urls = []
#     # for record in all_puuids_and_last_games:
#     #     urls.append(f"https://{record.region}.api.riotgames.com/lol/match/v5/matches/by-puuid/{record.puuid}/ids?start=0&count=1&queue=420")
#     # print(all_puuids_and_last_games)
#     try:
#         async with httpx.AsyncClient() as client:
#             response = await client.get(f'https://{"americas"}.api.riotgames.com/lol/match/v5/matches/by-puuid/{puuid}/ids?start=0&count=1&queue=420', headers=headers)
...
#         async with httpx.AsyncClient() as client:
#             for puuid in puuids:
#                 async with client.get(f'https://{"americas"}.api.riotgames.com/lol/match/v5/matches/by-puuid/{puuid}/ids?start=0&count=1&queue=420', headers=headers) as res:
#                     last_game = await res.json()
#                     data.append(last_game)
#             # tasks = [asyncio.create_task client.get(f'https://{"americas"}.api.riotgames.com/lol/match/v5/matches/by-puuid/{pid}/ids?start=0&count=1&queue=420', headers=headers) for pid in puuids]
#             # response = await client.get(f'https://{"americas"}.api.riotgames.com/lol/match/v5/matches/by-puuid/{puuid}/ids?start=0&count=1&queue=420', headers=headers)
#             # response.raise_for_status()
#             # data = response.json()
#     except httpx.HTTPError as e:
